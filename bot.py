...
def get_user_data(user_id):
    ...
    try:
        ...
        data = c.fetchone()
        if not data:
            return {"daily_count": 0, "servers": [], "first_name": "", "username": "", "wallet": 0, "warnings": 0, "is_banned": 0}
        return {
            "daily_count": data[1],
...
def get_main_keyboard(user_id):
    ...
    keyboard.add(
        create_custom_emoji_button("راهنما", "help", EMOJI_HELP)
    )
...
async def handle_callback(callback: CallbackQuery):
    ...
    if data.startswith("view_server_"):
        ...
        await callback.message.edit_text(
            f"سرور: {server['name']}\nوضعیت: فعال\nحجم: {server['volume']}",
            reply_markup=get_back_keyboard("back_to_services")
        )
        await callback.answer()
        return
    ...
    if data == "my_account":
        ...
        msg += f"نام: {user_data.get('first_name', 'کاربر')}\n"
        msg += f"یوزرنیم: @{user_data.get('username', 'ندارد')}\n"
        msg += f"تعداد سرورهای فعال: {len(user_servers)}\n"
        msg += f"سرورهای امروز: {user_data.get('daily_count', 0)}/2\n"
        msg += f"موجودی کیف پول: {wallet} تومان\n"
        msg += f"تعداد اخطارها: {user_data.get('warnings', 0)}"
...
1. دریافت سرور رایگان:
- هر کاربر روزانه 2 سرور دریافت میکنه
- حجم هر سرور: 1 گیگابایت
...
- مشاهده موجودی"""
    ...
    if data == "admin_stats" and int(user_id) == OWNER_ID:
        total_servers = len(get_free_servers())
        await callback.message.edit_text(
            f"تعداد کل سرورها: {total_servers}",
